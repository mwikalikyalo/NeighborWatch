...
class TestNeighborhodd(TestCase):
  def setUp(self):
      self.location = Location.objects.create(location='Machakos')
      self.hood = Neighborhood.objects.create(name='Maanzoni', location=self.location, securityline=2, hospitalhelpline=2, occupants=8)
...
class Post(TestCase):
    def setUp(self):
      ...
      self.post = Post.objects.create(posttitle='theives found', story='wezi wa tatu walipatikana', neighborhood=self.hood)
    ...
    def test_save_post(self):
      self.post2 = Post.objects.create(posttitle='wezi walipatikana ', story='thieves were found', neighborhood=self.hood)
